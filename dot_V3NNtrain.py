...
def convolutional_neural_network(x):
    weights = {'W_fc':tf.Variable(tf.random_normal([784,784])),
                'W_fc2':tf.Variable(tf.random_normal([784,1024])),
              'W_fc3':tf.Variable(tf.random_normal([1024,1024])),
                'out':tf.Variable(tf.random_normal([1024, n_classes]))}

    biases = {'b_fc':tf.Variable(tf.random_normal([784])),
    ...
    fc = tf.nn.relu(tf.matmul(x, weights['W_fc'])+biases['b_fc'])
